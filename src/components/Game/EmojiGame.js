...
class EmojiGame extends React.Component {
    constructor(props) {
        ...
        this.isWon = false;
        this.state = {
            score: 0, topScore: 0, gameState: "PLAYING", isThemeChanged: false,
            emojis: [
                {
                    id: "one",
                    isClicked: false,
    ...
    setTopScore = (value) => {
        ...
        if (topScore < value)
            this.setState({ topScore: value });
        else
            this.setState({ topScore: topScore });
        console.log("rest", this.state.topScore);
    }
    onChangeTheme = () => {
        this.setState(prevState => ({
            isThemeChanged: !prevState.isThemeChanged
        }));
        console.log(this.state.isThemeChanged);
    }
    render() {
        if (this.state.gameState === "PLAYING") {
            return (<div>
                <Navbar score={this.state.score} topScore={this.state.topScore} isThemeChanged={this.state.isThemeChanged} onChangeTheme={this.onChangeTheme} />
                <EmojiCard emojis={this.state.emojis} onEmojiClick={this.onEmojiClick} isWon={this.isWon} onPlayAgainClick={this.onPlayAgainClick} isThemeChanged={this.state.isThemeChanged} onChangeTheme={this.onChangeTheme} />
                <HowToPlay isThemeChanged={this.state.isThemeChanged} onChangeTheme={this.onChangeTheme} />
            </div >);
        }
        else if (this.state.gameState === "LOOSE") {
            return (<div>
                <Navbar score={this.state.score} topScore={this.state.topScore} isThemeChanged={this.state.isThemeChanged} onChangeTheme={this.onChangeTheme} />
                <LooseTheGame score={this.state.score} gameState={this.state.gameState} isWon={this.isWon} onPlayAgainClick={this.onPlayAgainClick} isThemeChanged={this.state.isThemeChanged} onChangeTheme={this.onChangeTheme} />
                <HowToPlay isThemeChanged={this.state.isThemeChanged} onChangeTheme={this.onChangeTheme} />
            </div >);
        }
