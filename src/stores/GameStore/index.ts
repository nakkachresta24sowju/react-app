...
class GameStore {
    ...
    onCellClick(cell, disabled) {
        if (cell.isHidden === true) {
            ...
            disabled = true;
            this.timerIdForWrongClick = window.setTimeout(this.goToInitialLevelAndUpdateCells, 100)
        }
    ...
    shuffleHiddenCells = () => {
        for (let i = this.currentLevelGridCells.length - 1; i > 0; i--) {
            ...
            [this.currentLevelGridCells[i], this.currentLevelGridCells[j]] = [this.currentLevelGridCells[j], this.currentLevelGridCells[i]];
        }
    }
